...
class StageTwoWinnerTiebreakOrganizerRankUnitTest {
	...
	@Test
	@DisplayName("Сумма и approval равны: у организатора лучший ранг у A — STAGE_TWO_TIEBREAK_BY_ORGANIZER_RANK")
	void tieOnSumAndApprovalBrokenByOrganizerPreference() {
		long restaurantA = 31L;
		long restaurantB = 32L;
		List<RoomStageOneFinalist> finalists = List.of(
			finalist(restaurantA, 4, 1),
			finalist(restaurantB, 4, 2)
		);
		...
		assertThat(outcome.principle()).isEqualTo(WinnerSelectionPrinciple.STAGE_TWO_TIEBREAK_BY_ORGANIZER_RANK);
	}
}
